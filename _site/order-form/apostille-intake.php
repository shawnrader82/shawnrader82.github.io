...
function nc_mkdir_if_missing($path, $baseUrl, $user, $pass) {
    $encodedPath = nc_encode_path($path);
    $url = rtrim($baseUrl, '/') . '/' . $encodedPath;
    $ch  = curl_init($url);
...
function nc_put_file($remotePath, $content, $baseUrl, $user, $pass) {
...
if (isset($_FILES['intake_upload_files'])) {
    error_log('DEBUG intake_upload_files=' . json_encode($_FILES['intake_upload_files']));
} else {
    error_log('DEBUG intake_upload_files MISSING');
}
if (isset($_FILES['shippinglabelupload'])) {
    error_log('DEBUG shippinglabelupload=' . json_encode($_FILES['shippinglabelupload']));
} else {
    error_log('DEBUG shippinglabelupload MISSING');
}
...
// Regular document uploads from Step 4 ("Upload your documents")
$uploadedFiles = [];
...
if (!empty($_FILES['intake_upload_files']['name'][0])) {
    foreach ($_FILES['intake_upload_files']['name'] as $idx => $origName) {
        ...
        if ($httpCode >= 400 || $httpCode === 0 || $response === false) {
            error_log("NC doc upload error $httpCode for $remotePath");
            error_log("NC doc response: " . substr((string)$response, 0, 200));
        } else {
            $uploadedFiles[] = [
...
// Prepaid shipping label upload (single file, Step 4 "Upload your prepaid shipping label")
$shippingLabelFile = null;

if (!empty($_FILES['shippinglabelupload']['name'])
    ...
    if ($httpCode >= 400 || $httpCode === 0 || $response === false) {
        error_log("NC label upload error $httpCode for $remotePath");
        error_log("NC label response: " . substr((string)$response, 0, 200));
    } else {
        $shippingLabelFile = [
            'originalName'  => $origName,
            'nextcloudPath' => $remotePath,
...
$hasTranslationService = in_array('translation', $services, true);
...
$hasSpeedOrAddons =
    ...
    $apostilleRushNotes !== '' ||
    $addonShippedHardCopy ||
    $addonNotarization ||
    $addonExpeditedTurnaround;
...
function send_via_gmail($to, $subject, $body, $replyToEmail = '', $replyToName = '')
{
    ...
    try {
        ...
        // From: orders, not Sean
        $mail->setFrom('orders@mobileamericannotary.com', 'Orders - Mobile American Notary');
...
if ($hasTranslationService) {
    $custBody .= '
          ...
          <tr>
            ...
            <td style="padding:8px 20px;border-bottom:1px solid #f0f0f0;">' . htmlspecialchars($translationApproxPages) . '</td>
          </tr>
          <tr>
            <td style="padding:8px 20px;border-bottom:1px solid #e0e0e0;color:#555;">Translation estimated total (USD)</td>
            <td style="padding:8px 20px;border-bottom:1px solid #e0e0e0;">' . htmlspecialchars($translationEstimatedTotal) . '</td>
          </tr>';
}
...
if ($hasSpeedOrAddons) {
    $custBody .= '
          ...
          <tr>
            ...
            <td style="padding:8px 20px;border-bottom:1px solid #f0f0f0;">' . htmlspecialchars($apostilleSpeedOption) . '</td>
          </tr>
          <tr>
            <td style="padding:8px 20px;border-bottom:1px solid #f0f0f0;color:#555;">Apostille rush notes</td>
            <td style="padding:8px 20px;border-bottom:1px solid #f0f0f0;">' . htmlspecialchars($apostilleRushNotes) . '</td>
          </tr>
          <tr>
...
<main class="page-wrap" style="min-height:60vh;">
  ...
  Thank you, <?php echo htmlspecialchars((string)$displayName, ENT_QUOTES, 'UTF-8'); ?>.
</h1>
